...
const NewTargetModal = ({ targets, setTargets, judges, staff, cases, preselectCaseId, onClose }) => {
  const [f, setF] = React.useState({ type: "Case Resolution", description: "", metric: 10, deadline: fmt(addDays(today, 30)), assignees: ["All Judges"], caseIds: preselectCaseId ? [preselectCaseId] : [] });
...
const BookingModal = ({ bookings, setBookings, cases, existing, preselectCaseId, onClose }) => {
  const [f, setF] = React.useState(existing || {
    caseId: preselectCaseId || cases[0].id, courtroom: COURTS[0],
