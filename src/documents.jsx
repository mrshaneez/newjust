...
function resolveTokens(template, caseData, opts = {}) {
  if (!template || !caseData) return template?.body || "";
  const c = caseData;
  const hearing = opts.hearing || null;
  ...
  // Next hearing — an explicit sitting (when issued from the schedule) wins,
  // otherwise the first future hearing on record, otherwise "to be fixed".
  const futureHearing = (c.hearings || [])
    .map((h) => h.date)
    .filter((d) => d && new Date(d) >= new Date(new Date().toDateString()))
    .sort()[0];
  const fmtLong = (d) => new Date(d).toLocaleDateString("en-GB",
    { weekday: "long", day: "2-digit", month: "long", year: "numeric" });
  const nextHearing = hearing && hearing.date
    ? fmtLong(hearing.date)
    : futureHearing
      ? fmtLong(futureHearing)
      : "a date to be fixed by the Registry";
  const hearingTime = hearing
    ? [hearing.timeStart, hearing.timeEnd].filter(Boolean).join("–")
    : "";
  const courtroom = hearing?.courtroom || "";
...
const TOKEN_LIST = [
  ...
  { key: "hearingTime",       label: "Hearing time" },
...
const GenerateDocModal = ({ template, cases, preselectCaseId, hearing, generatedDocs, setGeneratedDocs, onClose }) => {
  const { session } = useAuth();
  const toast = useToast();
  ...
  const [html, setHtml] = React.useState(() => bodyToHtml(resolveTokens(template, caseData, { hearing })));
  const prepHF = (str, cd) => {
    if (!str) return "";
    const resolved = resolveTokens({ body: str }, cd, { hearing });
  ...
  React.useEffect(() => {
    setHtml(bodyToHtml(resolveTokens(template, cases.find((c) => c.id === caseId), { hearing })));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [caseId, template.id]);
