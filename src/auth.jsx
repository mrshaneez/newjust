...
const AuthProvider = ({ children }) => {
  ...
  // Self-service reset: a user who forgot their password enters their
  // identifier. If an account exists, we issue a fresh temporary password and
  // force a change on next login — mirroring the registry's provisioning flow.
  // (In a real deployment the temp password would be emailed/SMSed rather than
  // shown on screen.) Admin password is fixed and cannot be reset here.
  const resetPassword = (identifier) => {
    const id = String(identifier || "").trim();
    if (!id) return { ok: false, error: "Enter the email or phone on your account." };
    const norm = (x) => String(x || "").trim().toLowerCase();
    if (norm(id) === norm(ADMIN_USER.username))
      return { ok: false, error: "The Global Admin password is fixed in this build and can't be reset." };
    const u = users.find((x) => norm(x.identifier) === norm(id));
    if (!u) return { ok: false, error: "No account found for that email or phone. Ask the registry to add you to the directory first." };

    const tempPassword = generateTempPassword();
    setUsers((p) => p.map((x) =>
      norm(x.identifier) === norm(id)
        ? { ...x, password: tempPassword, mustChangePassword: true, updatedAt: Date.now() }
        : x));
    return { ok: true, tempPassword, identifier: u.identifier };
  };
  ...
  return (
    <AuthCtx.Provider value={{
      ...
      provisionUser, changePassword, resetPassword,
    }}>
      {children}
    </AuthCtx.Provider>
  );
...
const AuthScreen = () => {
  ...
  const [resetResult, setResetResult] = React.useState(null); // { tempPassword, identifier }
  ...
  const submitReset = (e) => {
    e?.preventDefault();
    setErr(""); setBusy(true);
    const r = resetPassword(identifier);
    setBusy(false);
    if (!r.ok) { setErr(r.error); return; }
    setResetResult(r);
  };

  const backToSignin = (prefillPw = "") => {
    setMode("signin");
    setErr(""); setResetResult(null);
    setPassword(prefillPw);
  };
      ...
      <div style={{
        ...
        <form onSubmit={submit} style={{ width: "100%", maxWidth: 380 }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: -6, marginBottom: 14 }}>
            <button type="button" onClick={() => { setMode("reset"); setErr(""); setPassword(""); }}
        ...
        <form onSubmit={submitReset} style={{ width: "100%", maxWidth: 380 }}>
          <div style={{ marginBottom: 28 }}>
            <div style={{ fontSize: 22, fontWeight: 500, letterSpacing: "-0.02em" }}>
              Reset password
            </div>
            <div style={{ fontSize: 13, color: "var(--text-2)", marginTop: 4, lineHeight: 1.5 }}>
              Enter the email or phone on your account. We'll issue a temporary
              password you can use to sign in and set a new one.
            </div>
          </div>
          ...
          {!resetResult ? (
            <>
              <Input label="Email · phone"
                placeholder="you@highcourt.gov.mv"
                value={identifier} onChange={(e) => setIdentifier(e.target.value)} autoFocus/>
              ...
              <Btn type="submit" variant="primary" size="lg" onClick={submitReset}
                style={{ width: "100%", justifyContent: "center", marginTop: 4, marginBottom: 14 }}>
                {busy ? "Please wait…" : "Send temporary password"}
              </Btn>

              <button type="button" onClick={() => backToSignin()}
                style={{ background: "transparent", border: "none", cursor: "pointer",
                  color: "var(--text-2)", fontSize: 12.5, fontWeight: 500, padding: 0,
                  display: "flex", alignItems: "center", gap: 5, margin: "0 auto" }}>
                <I.chevronL size={13}/> Back to sign in
              </button>
            </>
          ) : (
            <>
              <div style={{
                ...
                border: "1px solid rgba(45, 90, 61, 0.2)", borderRadius: 8,
                fontSize: 12.5, lineHeight: 1.6,
              }}>
                <div style={{ fontWeight: 600, marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}>
                  <I.check size={14}/> Temporary password issued
                </div>
                <div style={{ color: "var(--text-2)" }}>
                  For <span style={{ fontWeight: 500, color: "var(--text)" }}>{resetResult.identifier}</span>.
                  In a live system this would be sent to you privately. Use it to sign in, then set a new password.
                </div>
                <div style={{
                  ...
                  <span className="mono" style={{ fontSize: 16, fontWeight: 600,
                    letterSpacing: "0.08em", color: "var(--text)" }}>{resetResult.tempPassword}</span>
                  <button type="button"
                    onClick={() => { navigator.clipboard?.writeText(resetResult.tempPassword); }}
                    style={{ background: "transparent", border: "1px solid var(--line)",
                      borderRadius: 5, padding: "4px 9px", fontSize: 11.5, cursor: "pointer",
                      color: "var(--text-2)" }}>Copy</button>
                </div>
              </div>

              <Btn variant="primary" size="lg" type="button"
                onClick={() => backToSignin(resetResult.tempPassword)}
