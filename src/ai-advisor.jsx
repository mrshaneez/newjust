...
const AIAdvisor = ({ open, onClose, cases, targets, bookings }) => {
  ...
  // ── Panel size: resizable width + maximize, both persisted ──
  const MIN_W = 340, MAX_W = 1100;
  const [width, setWidth] = React.useState(() => {
    const v = parseInt(localStorage.getItem("jd_advisor_width") || "", 10);
    return Number.isFinite(v) ? Math.min(MAX_W, Math.max(MIN_W, v)) : 380;
  });
  const [maximized, setMaximized] = React.useState(() => localStorage.getItem("jd_advisor_max") === "1");
  const [resizing, setResizing] = React.useState(false);
  React.useEffect(() => { localStorage.setItem("jd_advisor_width", String(width)); }, [width]);
  React.useEffect(() => { localStorage.setItem("jd_advisor_max", maximized ? "1" : "0"); }, [maximized]);
  ...
  const startResize = (e) => {
    ...
    const onMove = (ev) => {
      ...
      // Panel is docked right; width grows as the pointer moves left.
      const w = Math.min(MAX_W, Math.max(MIN_W, window.innerWidth - x));
      setWidth(w);
    };
    const stop = () => {
  ...
  // Effective width: maximized fills most of the viewport (capped)
  const effWidth = maximized ? "min(960px, 82vw)" : width;
  ...
  return (
    <aside style={{
      width: effWidth, flexShrink: 0,
      background: "var(--paper)",
      borderLeft: "1px solid var(--line)",
      display: "flex", flexDirection: "column",
      position: "relative",
      transition: resizing ? "none" : "width .18s ease",
    }}>
      {/* Left-edge resize handle */}
      <div onPointerDown={startResize} onDoubleClick={() => setMaximized((v) => !v)}
        title="Drag to resize · double-click to maximise"
        style={{
          position: "absolute", left: -4, top: 0, bottom: 0, width: 9, zIndex: 20,
          cursor: "ew-resize", display: "flex", alignItems: "center", justifyContent: "center",
        }}>
        <span style={{
          width: 4, height: 44, borderRadius: 99,
          background: resizing ? "var(--accent)" : "var(--line-2)",
      ...
      <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--line)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 2, flexShrink: 0 }}>
          ...
          <button onClick={() => setMaximized((v) => !v)} title={maximized ? "Restore size" : "Maximise"} style={advisorIconBtn}
            onMouseEnter={(e)=>e.currentTarget.style.background="var(--paper-2)"}
            onMouseLeave={(e)=>e.currentTarget.style.background="transparent"}>
            {maximized ? <I.collapse size={14}/> : <I.expand size={14}/>}
          </button>
