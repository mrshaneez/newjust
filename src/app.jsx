...
// State that survives reloads — persisted to localStorage, rehydrated on load.
// Anything the user creates/edits (cases, people, documents, …) lives in these.
function usePersistentState(key, initial) {
  const [val, setVal] = React.useState(() => {
    try {
      const raw = localStorage.getItem(key);
      if (raw != null) return JSON.parse(raw);
    } catch (e) { /* corrupt / unavailable — fall back to seed */ }
...
function AppInner({ onChangePassword }) {
  ...
  const [allCases, setCases] = usePersistentState("jd_cases_v1", initCases);
  const [targets, setTargets] = usePersistentState("jd_targets_v1", initTargets);
  const [bookings, setBookings] = usePersistentState("jd_bookings_v1", initBookings);
  const [judges, setJudges] = usePersistentState("jd_judges_v1", initJudges);
  const [lawyers, setLawyers] = usePersistentState("jd_lawyers_v1", initLawyers);
  const [parties, setParties] = usePersistentState("jd_parties_v1", initParties);
  const [representatives, setRepresentatives] = usePersistentState("jd_reps_v1", initRepresentatives);
  const [staff, setStaff] = usePersistentState("jd_staff_v1", initStaff);
  const [sections, setSections] = usePersistentState("jd_sections_v1", initSections);
  const [accessGrants, setAccessGrants] = usePersistentState("jd_grants_v1", initAccessGrants);
  const [templates, setTemplates] = usePersistentState("jd_templates_v2", () => {
    // Migrate from v1: refresh built-in templates from code, keep user uploads.
    try {
      const v1 = JSON.parse(localStorage.getItem("jd_templates_v1") || "null");
      if (Array.isArray(v1)) return [...initTemplates, ...v1.filter((t) => t.uploaded)];
    } catch (e) {}
    return initTemplates;
  });
  const [generatedDocs, setGeneratedDocs] = usePersistentState("jd_docs_v1", []);
  const [statusSets, setStatusSets] = usePersistentState("jd_statussets_v1", buildInitialStatusSets);
  ...
  React.useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      // g then d/c/t — quick navigation
      if (k === "g") { gPending = true; clearTimeout(gTimer); gTimer = setTimeout(() => gPending = false, 800); return; }
      if (gPending) {
        gPending = false;
        const dest = { d: "dashboard", c: "cases", t: "tasks" }[k];
  ...
  const counts = {
    ...
    chat: (typeof window.chatUnreadCount === "function" ? window.chatUnreadCount(session) : 0),
  };
    ...
    <div style={{ display: "flex", height: "100vh", overflow: "hidden", background: "var(--paper-2)" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "var(--paper)" }}>
        <Topbar page={page} viewCase={viewCase} breadcrumbActions={actions}
          cases={cases} bookings={bookings} navigate={navigate} goCase={goCase}/>
        <main style={{ flex: 1, minWidth: 0, overflowY: "auto", padding: "24px 28px", background: "var(--paper-2)" }}>
          ...
          {page === "cases" && <CasesPage cases={cases} setCases={setCases} viewCase={viewCase} setViewCase={setViewCase} updateCase={updateCase} targets={targets} bookings={bookings} setModal={setModal}/>}
          {page === "courtrooms" && <CourtroomsPage bookings={bookings} setBookings={setBookings} cases={cases} setModal={setModal}/>}
          {page === "calendar" && <CalendarPage cases={cases} bookings={bookings} targets={targets} judges={judges} staff={staff} sections={sections} accessGrants={accessGrants} setModal={setModal} goCase={goCase}/>}
          ...
          {page === "chat" && <ChatPage cases={cases} judges={judges} staff={staff} parties={parties} representatives={representatives} lawyers={lawyers} sections={sections} accessGrants={accessGrants} updateCase={updateCase} goCase={goCase}/>}
      ...
      {(modal === "newTarget" || modal?.type === "newTarget") && <NewTargetModal targets={targets} setTargets={setTargets} judges={judges} staff={staff} cases={cases} preselectCaseId={modal?.caseId} onClose={() => setModal(null)}/>}
      ...
      {(modal === "newBooking" || modal?.type === "newBooking") && <BookingModal bookings={bookings} setBookings={setBookings} cases={cases} preselectCaseId={modal?.caseId} onClose={() => setModal(null)}/>}
      ...
      {modal?.type === "generateDoc" && <GenerateDocModal template={modal.template} cases={cases} preselectCaseId={modal.caseId} hearing={modal.hearing} generatedDocs={generatedDocs} setGeneratedDocs={setGeneratedDocs} onClose={() => setModal(null)}/>}
      {modal?.type === "pickTemplate" && <PickTemplateModal templates={templates} filterCategory={modal.filterCategory} filterName={modal.filterName} preselectCaseId={modal.caseId} onPick={(tpl) => setModal({ type: "generateDoc", template: tpl, caseId: modal.caseId, hearing: modal.hearing })} onClose={() => setModal(null)}/>}
